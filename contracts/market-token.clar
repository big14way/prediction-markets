;; market-token.clar
;; SIP-010 compliant token for prediction market rewards/governance
...
(define-data-var token-name (string-ascii 32) "Prediction Market Token")
(define-data-var token-symbol (string-ascii 10) "PRED")
...
(define-read-only (get-balance (account principal))
    (ok (ft-get-balance market-token account)))
...
(define-public (burn (amount uint))
    (ft-burn? market-token amount tx-sender))
